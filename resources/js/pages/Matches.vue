        ...
        <header
            ...
            <div class="flex items-center space-x-3">
                ...
                <div>
                    ...
                    <p class="text-xs font-medium text-gray-500 ">Football Predictions</p>
                </div>
            </div>
            <div class="hidden md:flex items-center space-x-4">
                ...
                <button
                    class="p-2 bg-gray-100 text-gray-600 rounded-full hover:bg-gray-200 transition  :bg-gray-700  :hover:bg-gray-600">
...
<div
  class="rounded-2xl bg-white  :bg-gray-900 shadow-lg ring-1 ring-gray-200  :ring-gray-700 p-6 mb-10 transition-all duration-300">
  <!-- Header -->
  <div class="flex items-center justify-between mb-6">
    <div class="flex items-center space-x-2">
      <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 text-indigo-600  :text-indigo-400" fill="none"
      ...
      <h3 class="text-xl font-semibold text-gray-800  :text-gray-100">Advanced Filters</h3>
    </div>
    <button @click="resetFilters"
      class="text-sm bg-gray-100  :bg-gray-800 text-gray-600  :text-gray-300 px-3 py-1.5 rounded-full hover:bg-gray-200  :hover:bg-gray-700 transition font-medium">
      Reset All
    </button>
  </div>
  ...
  <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
    <!-- League Select -->
    <div>
      <label class="text-sm font-medium text-gray-700  :text-gray-300 mb-1 block">League</label>
      <div class="relative">
        <select v-model="filters.league"
          class="w-full appearance-none bg-gray-50  :bg-gray-800 text-gray-800  :text-gray-100 border border-gray-200  :border-gray-700 rounded-xl px-4 py-2 pr-10 focus:ring-2 focus:ring-indigo-500 text-sm">
    ...
    <!-- Prediction Type -->
    <div>
      <label class="text-sm font-medium text-gray-700  :text-gray-300 mb-1 block">Prediction Type</label>
      <div class="relative">
    ...
    <!-- Match Date Filter -->
    <div>
      <label class="text-sm font-medium text-gray-700  :text-gray-300 mb-1 block">Match Date</label>
      <div class="flex gap-2">
        <button @click="setDateFilter('yesterday')"
          :class="[filters.matchDate === 'yesterday' ? 'bg-indigo-100  :bg-indigo-900 text-indigo-700  :text-indigo-300 border-indigo-300' : 'bg-white  :bg-gray-800 text-gray-700  :text-gray-300 border-gray-200  :border-gray-600', 'flex-1 py-1.5 text-xs rounded-full border text-center transition font-medium']">
          Yesterday
        </button>
        <button @click="setDateFilter('today')"
          :class="[filters.matchDate === 'today' ? 'bg-indigo-100  :bg-indigo-900 text-indigo-700  :text-indigo-300 border-indigo-300' : 'bg-white  :bg-gray-800 text-gray-700  :text-gray-300 border-gray-200  :border-gray-600', 'flex-1 py-1.5 text-xs rounded-full border text-center transition font-medium']">
          Today
        </button>
        <button @click="setDateFilter('tomorrow')"
          :class="[filters.matchDate === 'tomorrow' ? 'bg-indigo-100  :bg-indigo-900 text-indigo-700  :text-indigo-300 border-indigo-300' : 'bg-white  :bg-gray-800 text-gray-700  :text-gray-300 border-gray-200  :border-gray-600', 'flex-1 py-1.5 text-xs rounded-full border text-center transition font-medium']">
          Tomorrow
        </button>
      </div>
    </div>
...
<div class="md:col-span-2">
  <label class="block text-sm font-medium text-gray-700  :text-gray-300 mb-2">Match Time Range (24h)</label>
  <div class="flex space-x-4 items-center">
    <div class="flex-1">
      <input type="range" min="0" max="23" v-model.number="filters.startHour"
        class="w-full accent-indigo-600 h-2 bg-gray-200  :bg-gray-700 rounded-lg cursor-pointer" />
      <p class="text-xs mt-1 text-gray-600  :text-gray-400">Start Hour: {{ filters.startHour }}:00</p>
    </div>
    <div class="flex-1">
      <input type="range" min="0" max="23" v-model.number="filters.endHour"
        class="w-full accent-indigo-600 h-2 bg-gray-200  :bg-gray-700 rounded-lg cursor-pointer" />
      <p class="text-xs mt-1 text-gray-600  :text-gray-400">End Hour: {{ filters.endHour }}:00</p>
    </div>
  </div>
</div>
  ...
  <div class="text-right">
    <button @click="showAdvancedFilters = !showAdvancedFilters"
      class="text-sm text-indigo-600  :text-indigo-400 hover:underline font-medium">
  ...
  <transition name="fade">
    <div v-if="showAdvancedFilters"
      class="mt-6 border-t pt-6 border-gray-200  :border-gray-700 grid grid-cols-1 md:grid-cols-3 gap-6">
 <!-- Odds Range Sliders -->
<div class="flex items-center space-x-4">
  <div class="flex-1">
    <input
      ...
      class="w-full h-2 bg-gray-200  :bg-gray-700 rounded-lg cursor-pointer accent-indigo-600"
    />
    <p class="text-xs mt-1 text-gray-600  :text-gray-400">Min Odds: {{ filters.minOdds }}</p>
  </div>
  <div class="flex-1">
    <input
      type="range"
      ...
      class="w-full h-2 bg-gray-200  :bg-gray-700 rounded-lg cursor-pointer accent-indigo-600"
    />
    <p class="text-xs mt-1 text-gray-600  :text-gray-400">Max Odds: {{ filters.maxOdds }}</p>
  </div>
</div>
      ...
      <!-- Ranking Type -->
      <div>
        <label class="block text-sm font-medium text-gray-700  :text-gray-300 mb-1">Team Ranking</label>
        <div class="flex gap-2">
          <button v-for="type in ['all', 'topVsTop', 'mismatch']" :key="type" @click="setRankingFilter(type)"
            :class="[
              filters.rankingType === type
                ...
                : 'bg-white  :bg-gray-800 text-gray-700  :text-gray-300 border-gray-200  :border-gray-600',
      ...
      <!-- Team Search -->
      <div>
        <label class="block text-sm font-medium text-gray-700  :text-gray-300 mb-1">Search Teams</label>
        <div class="relative">
          <input type="text" v-model="filters.teamSearch" placeholder="Enter team name..."
            class="w-full px-10 py-2 bg-gray-50  :bg-gray-800 border border-gray-200  :border-gray-700 rounded-xl text-sm text-gray-800  :text-white" />
  ...
  <!-- Over 2.5 -->
  <label class="flex items-center space-x-2 text-sm font-medium text-gray-700  :text-gray-300">
    <input type="checkbox" v-model="filters.over25"
      class="form-checkbox rounded text-indigo-600  :bg-gray-800  :border-gray-600">
...
<label class="flex items-center space-x-2 text-sm font-medium text-gray-700">
  <input type="checkbox" v-model="filters.bigRankGap" class="form-checkbox text-indigo-600">
  <span>💣 Big Rank Gap (≥10 places)</span>
</label>
        ...
        <main class="">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
                <div v-for="match in filteredMatches" :key="match.id"
                    class="bg-white  :bg-gray-850 rounded-2xl overflow-hidden shadow-lg hover:shadow-xl transition-all duration-300 group border border-gray-100  :border-gray-700 hover:border-gray-200  :hover:border-gray-600">

                    <!-- Match Header -->
                    <div
                                    class="relative p-6 pb-4 bg-gradient-to-r from-blue-50 to-indigo-50  :from-gray-800  :to-gray-800">
                                    <div class="absolute top-4 right-4 bg-white  :bg-gray-700 px-2 py-1 rounded-full shadow-xs text-right space-y-0.5">
            <div class="text-[11px] font-semibold text-indigo-600  :text-indigo-300">
                🕒 {{ match.match_time }}
            </div>
            <div class="text-[10px] text-gray-500  :text-gray-400">
                📅 {{ match.match_date }}
            </div>
            </div>
                        ...
                        <div
                            class="text-xs font-medium text-indigo-500  :text-indigo-400 mb-1 uppercase tracking-wider">
                            {{ match.league }}
                        </div>

                        <h3 class="text-2xl font-extrabold text-center">
                            <span
                                class="text-gray-900  :text-white group-hover:text-blue-600  :group-hover:text-blue-400 transition-colors">
                                {{ match.home_team }}
                            </span>
                            <span class="mx-2 text-gray-400  :text-gray-500 font-normal">vs</span>
                            <span
                                class="text-gray-900  :text-white group-hover:text-red-600  :group-hover:text-red-400 transition-colors">
                                {{ match.away_team }}
                            </span>
                        </h3>
                    </div>
                    ...
                    <!-- Odds Section -->
                    <div class="px-6 py-4 bg-gray-50  :bg-gray-800/50">
                        <div class="grid grid-cols-3 gap-3">
  <!-- Home -->
  <div
    :class="[
      'rounded-xl p-3 text-center border transition-shadow relative',
      'bg-gradient-to-b from-blue-50 to-white  :from-blue-900/20  :to-gray-800',
      'border-blue-100  :border-blue-800/50 hover:shadow-md',
      getPrediction(match) === 'strong home win' ? 'ring-2 ring-blue-500  :ring-blue-400' : ''
    ]">
    <div class="text-xs font-semibold text-blue-600  :text-blue-300 mb-1">Home</div>
    <div class="text-xl font-bold text-gray-900  :text-white">@{{ match.odds_home }}</div>
    <div v-if="getPrediction(match) === 'strong home win'" class="mt-1 text-[10px] text-blue-700  :text-blue-300 font-semibold">
      🏠 Prediction: 1 <br>
  ...
  <div
    :class="[
      'rounded-xl p-3 text-center border transition-shadow relative',
      'bg-gradient-to-b from-gray-50 to-white  :from-gray-700  :to-gray-800',
      'border-gray-200  :border-gray-700 hover:shadow-md',
      getPrediction(match) === 'potential draw' ? 'ring-2 ring-gray-500  :ring-gray-300' : ''
    ]">
    <div class="text-xs font-semibold text-gray-600  :text-gray-300 mb-1">Draw</div>
    <div class="text-xl font-bold text-gray-900  :text-white">@{{ match.odds_draw }}</div>
    <div v-if="getPrediction(match) === 'potential draw'" class="mt-1 text-[10px] text-gray-700  :text-gray-300 font-semibold">
      🤝 Prediction: X <br>
      Confidence: {{ getConfidence(match) }}
    </div>
  </div>
  ...
  <div
    :class="[
      'rounded-xl p-3 text-center border transition-shadow relative',
      'bg-gradient-to-b from-red-50 to-white  :from-red-900/20  :to-gray-800',
      'border-red-100  :border-red-800/50 hover:shadow-md',
      getPrediction(match) === 'likely away win' ? 'ring-2 ring-red-500  :ring-red-400' : ''
    ]">
    <div class="text-xs font-semibold text-red-600  :text-red-300 mb-1">Away</div>
    <div class="text-xl font-bold text-gray-900  :text-white">@{{ match.odds_away }}</div>
    <div v-if="getPrediction(match) === 'likely away win'" class="mt-1 text-[10px] text-red-700  :text-red-300 font-semibold">
      🛫 Prediction: 2 <br>
      🔥 Confidence: {{ getConfidence(match) }}
    </div>
  </div>
                    ...
                    <div v-if="match.details" class="px-6 py-3 border-t border-gray-100  :border-gray-700">
                        <div
                            class="flex items-center justify-between bg-indigo-50/50  :bg-indigo-900/20 p-3 rounded-lg">
                            <div class="flex items-center">
                                <div class="w-3 h-3 rounded-full bg-indigo-500 mr-2"></div>
                                <span class="text-sm font-medium text-gray-700  :text-gray-300">AI Prediction</span>
                            </div>
                            <span class="font-bold text-indigo-600  :text-indigo-400">{{ getPrediction(match)
                            }}</span>
                        </div>
                    </div>
                    ...
                    <div v-if="match.details" class="p-6 pt-4">
                        <div class="flex items-center mb-4">
                            ...
                            <h3 class="text-sm font-bold text-gray-800  :text-gray-200">TEAM COMPARISON</h3>
                        </div>
                        ...
                        <div class="space-y-5">
                            ...
                            <div class="relative">
                                ...
                                <div class="pl-6">
                                    <div class="flex items-center justify-between mb-2">
                                        <h4 class="font-bold text-gray-900  :text-gray-100">{{ match.home_team }}
                                        </h4>
                                        <span
                                            class="text-xs font-medium bg-blue-100  :bg-blue-900/30 text-blue-600  :text-blue-300 px-2 py-1 rounded-full">
                                    ...
                                    <div class="grid grid-cols-4 gap-2 text-xs mb-3">
                                        ...
                                        <div class="text-center">
                                            <div class="font-bold text-green-600  :text-green-400">{{
                                        ...
                                        <div class="text-center">
                                            <div class="font-bold text-indigo-600  :text-indigo-400">{{
                                    ...
                                    <!-- Enhanced Stats Row -->
                                    <div class="grid grid-cols-3 gap-2 bg-gray-50  :bg-gray-800/30 p-2 rounded-lg">
                                        <div class="text-center">
                                            <div class="text-lg font-bold text-gray-900  :text-white">{{
                                        ...
                                        <div class="text-center">
                                            <div class="text-lg font-bold text-indigo-600  :text-indigo-400">{{
                                                match.details.home_pts }}</div>
                            ...
                            <div class="relative">
                                ...
                                <div class="pl-6">
                                    <div class="flex items-center justify-between mb-2">
                                        <h4 class="font-bold text-gray-900  :text-gray-100">{{ match.away_team }}
                                        </h4>
                                        <span
                                            class="text-xs font-medium bg-red-100  :bg-red-900/30 text-red-600  :text-red-300 px-2 py-1 rounded-full">
                    ...
                    <div v-if="match.details && match.details.h2h_home_wins !== null"
                        class="px-6 py-4 bg-gray-50  :bg-gray-800/50 border-t border-gray-100  :border-gray-700">
                        <div class="flex items-center justify-between mb-4">
                            <div class="flex items-center">
                                ...
                                <h3 class="text-sm font-bold text-gray-800  :text-gray-200">HEAD-TO-HEAD</h3>
                            </div>
                            <div class="flex items-center space-x-4">
                                <div class="text-center">
                                    <div class="text-xs text-gray-500  :text-gray-400">Home</div>
                                    <div class="font-bold text-blue-600  :text-blue-400">{{
                                ...
                                <div class="text-center">
                                    <div class="text-xs text-gray-500  :text-gray-400">Away</div>
                                    <div class="font-bold text-red-600  :text-red-400">{{ match.details.h2h_away_wins
                                    }}</div>
                                </div>
                            </div>
                        </div>
                        ...
                        <a :href="match.match_link" target="_blank" rel="noopener"
                            class="w-full flex items-center justify-center py-2.5 px-4 bg-white  :bg-gray-800 border border-gray-200  :border-gray-700 rounded-lg text-sm font-medium text-indigo-600  :text-indigo-400 hover:bg-indigo-50  :hover:bg-indigo-900/30 transition-colors">
                            View Full Analysis
